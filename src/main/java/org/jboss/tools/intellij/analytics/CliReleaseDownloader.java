...
public final class CliReleaseDownloader {
  ...
  public CliReleaseDownloader(final String tarBallName, final String binaryName, final ICookie cookies, final String repoName) throws IOException {
  ...
  public File download(final ProgressIndicator indicator) throws IOException {
    ...
    tarBallName = tarBallName.replace("version",
            latestReleaseTag.replace("v", ""));
    ...
    if (!isNewRelease(latestReleaseTag) && tarBallDest.exists()) {
      return binaryDest;
    }

    final ActionMessage telemetry = TelemetryService.instance()
            .action("cli:download").property("cliVersion", latestReleaseTag);
    try {
      final String url = this.release.getDownloadUri(latestReleaseTag, this.tarBallName);
      HttpRequests
        .request(url)
